...
results <- df %>%
  ...
  summarize(days = sum(n),
            ...
            .groups = 'drop') %>%
  select(-floods_p_d) %>%
  summarize(across(contains('floods'), c(mean, sd), na.rm = TRUE)) %>%
  rename_with( ~ sub('1', 'mean', sub('2', 'sd', .x)))
...
```{r mgs_analysis}
f_flood_counts <- function(dat, dts, observed_wl, slr = 0, flood = FLOOD_ELEVATION) {
  # dat is a dataframe
  ...
  df <- tibble(theDate = dat[[dts]], obs_wl = dat[[obs_wl]]) %>%
    mutate(Year = as.numeric(format(theDate, format = '%Y')),
           exceeds_0 = obs_wl > flood)

  if(! ((length(slr) == 1 & slr[[1]] == 0)   | 
       (length(slr) == 1  & is.na(slr[[1]])) |
        length(slr) == 0)) {
  # then we have at least one value to add to the null SLR case
  # Is there a way to do this inside the tidyverse?
    for (n in seq_along(slr)) {
      if(slr[[n]] != 0) {               # don't add a pointless no SLR column
  ...
  # Now we run through the calculation steps.
   result <- df %>%
     # Group by day
     group_by(theDate) %>%
     select(-contains('slr')) %>%
  ...
  # add an attribute that retains the SLR values used.
  attr(result, 'slr') <- slr
  return(result)
}
...
```{r mgs_means}
mgs_est <- f_flood_counts(epoch, theDate, MLLW, c(0.3048, 0.6096, 0.9144))
mgs_est
```
...
```{r mgs_means_recent}
mgs_est_modern <- f_flood_counts(epoch, theDate, MLLW,
...
### Direct Substitution
We ran these calculations previously using the MGS approach. 
...
mgs_est_modern
```
(Note here the SD is an interannual SD).

We can calculate an expected ratio of increased flooding events, and build a
useful vector for later use.
```{r build_vector_of_mgs_results}
...
## ARIMA
We create a wrapper function to automate running our simulation repeatedly for
each sea level scenario.  As we did for the 
...
We run this simulation for the same sea level rise scenarios we just used.  The
code here uses `lapply()` to apply a function over a list, and return a list, 
here a list of vectors containing results of separate simulation runs.
